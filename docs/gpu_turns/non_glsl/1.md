...
pub const FACT_VALUE_R0: i32 = 2;   // lower 16 bits
...
export fn main() callconv(.spirv_kernel) void {
    ...
    gpu.binding(&embedding_table, shared.SET_MODEL, shared.BIND_EMBEDDING);
    gpu.binding(&layer_weights, shared.SET_MODEL, shared.BIND_LAYER_WEIGHTS);
    gpu.binding(&lm_head_weights, shared.SET_MODEL, shared.BIND_LM_HEAD);
    gpu.binding(&ln_params, shared.SET_MODEL, shared.BIND_LN_PARAMS);

    gpu.binding(&kb_store, shared.SET_KB_DATA, shared.BIND_KB_STORE);
    gpu.binding(&fact_store, shared.SET_KB_DATA, shared.BIND_FACT_STORE);
    gpu.binding(&rule_store, shared.SET_KB_DATA, shared.BIND_RULE_STORE);
    gpu.binding(&term_store, shared.SET_KB_DATA, shared.BIND_TERM_STORE);
    gpu.binding(&live_state, shared.SET_KB_DATA, shared.BIND_LIVE_STATE);

    gpu.binding(&scratch_a, shared.SET_SCRATCH, shared.BIND_SCRATCH_A);
    gpu.binding(&scratch_b, shared.SET_SCRATCH, shared.BIND_SCRATCH_B);
    gpu.binding(&kv_cache, shared.SET_SCRATCH, shared.BIND_KV_CACHE);

    gpu.binding(&params, shared.SET_CONTROL, shared.BIND_PARAMS);
    gpu.binding(&status_buf, shared.SET_CONTROL, shared.BIND_STATUS);
    gpu.binding(&result_counts, shared.SET_CONTROL, shared.BIND_RESULT_COUNTS);
...
fn op_kv_cache_append() void {
    ...
    var rem = idx;
    const kv_sel = @mod(rem, 2); rem = @divTrunc(rem, 2);
    const dim = @mod(rem, d_head); rem = @divTrunc(rem, d_head);
    const head = @mod(rem, n_heads); rem = @divTrunc(rem, n_heads);
    const token = rem;
...
fn op_rule_match_scan() void {
    ...
    if (rule_id == -1) { status_buf.data[@intCast(idx)] = 0; return; }
...
fn op_rule_body_eval() void {
    ...
    if (rule_idx >= n_matched) { scratch_b.data[@intCast(idx)] = 1; return; }
    ...
    if (body_idx >= body_count) { scratch_b.data[@intCast(idx)] = 1; return; }
...
fn op_rule_check_satisfied() void {
    ...
    while (bi < body_count and bi < max_body) : (bi += 1) {
        if (scratch_b.data[@intCast(eval_base + bi)] == 0) { all = false; break; }
    }
...
fn op_builtin_binary() void {
    ...
    const result = switch (sub) {
        ...
        6 => if (a > b) a else b,
        7 => gcd_iter(a, b),
        8 => blk: { const g = gcd_iter(a, b); break :blk if (g != 0) @as(i32, @intCast(@divTrunc(@as(i64, a), g) * b)) else 0; },
        9 => a & b,
        10 => a | b,
        11 => a ^ b,
    ...
    while (stride > 0) : (stride >>= 1) {
        if (local < stride) {
            switch (sub) {
                ...
                2, 9 => { if (s_i64[local + stride] < s_i64[local]) { s_i64[local] = s_i64[local + stride]; s_idx[local] = s_idx[local + stride]; } },
                3, 10 => { if (s_i64[local + stride] > s_i64[local]) { s_i64[local] = s_i64[local + stride]; s_idx[local] = s_idx[local + stride]; } },
