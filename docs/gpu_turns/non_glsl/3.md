...
Turn 2 complete. Single pipeline, single shader module, `DispatchRequest` takes a `ParamsBuffer` (op_code baked in), typed upload/download helpers for Fact/Term/Rule/Kb, `dispatchClean` resets status before dispatch, `dispatch1D`/`dispatch2D` convenience functions. All 20 host modules now use `bridge.dispatch(&.{ .params = gpu_params.someOp(...), .group_count_x = bridge.workgroups(n) })` pattern.

All files written. Full system:
- `vlp_gpu_shared.zig` — SPIR-V-safe constants + enums
- `vlp_kernel.zig` — single GPU kernel, 28 ops
- `vlp_types.zig` — host types with `toInts`/`fromInts` serialization
- `vlp_gpu_params.zig` — `ParamsBuffer` builders per op
- `vlp_bridge.zig` — single-pipeline Vulkan bridge
- `build.zig` + `build.zig.zon` — SPIR-V compile + embed + host build
- 20 host modules from turns 1-7 (find-and-replace `PipelineId`→`OpCode`)
